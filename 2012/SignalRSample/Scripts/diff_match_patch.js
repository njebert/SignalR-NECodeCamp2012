...
b, d) : this.diff_bisect_(a, b, d)
    ...
    diff_match_patch.prototype.diff_bisect_ = function (a, b, c) {
        for (var d = a.length, e = b.length, f = Math.ceil((d + e) / 2), g = f, h = 2 * f, j = Array(h), i = Array(h), k = 0; k < h; k++) j[k] = -1, i[k] = -1; j[g + 1] = 0; i[g + 1] = 0; for (var k = d - e, p = k % 2 != 0, q = 0, t = 0, o = 0, v = 0, u = 0; u < f; u++) {
            if ((new Date).getTime() > c) break; for (var n = -u + q; n <= u - t; n += 2) {
                var l = g + n, m; m = n == -u || n != u && j[l - 1] < j[l + 1] ? j[l + 1] : j[l - 1] + 1; for (var r = m - n; m < d && r < e && a.charAt(m) == b.charAt(r); ) m++, r++; j[l] = m; if (m > d) t += 2; else if (r > e) q += 2; else if (p && (l = g + k - n, l >= 0 && l < h && i[l] != -1)) {
                    var s = d - i[l];
                    if (m >= s) return this.diff_bisectSplit_(a, b, m, r, c)
                } 
            } for (n = -u + o; n <= u - v; n += 2) { l = g + n; s = n == -u || n != u && i[l - 1] < i[l + 1] ? i[l + 1] : i[l - 1] + 1; for (m = s - n; s < d && m < e && a.charAt(d - s - 1) == b.charAt(e - m - 1); ) s++, m++; i[l] = s; if (s > d) v += 2; else if (m > e) o += 2; else if (!p && (l = g + k - n, l >= 0 && l < h && j[l] != -1 && (m = j[l], r = g + m - l, s = d - s, m >= s))) return this.diff_bisectSplit_(a, b, m, r, c) } 
        } return [[-1, a], [1, b]]
    };
    ...
    diff_match_patch.prototype.diff_halfMatch_ = function (a, b) {
        function c(a, b, c) { for (var d = a.substring(c, c + Math.floor(a.length / 4)), e = -1, g = "", h, j, n, l; (e = b.indexOf(d, e + 1)) != -1; ) { var m = f.diff_commonPrefix(a.substring(c), b.substring(e)), r = f.diff_commonSuffix(a.substring(0, c), b.substring(0, e)); g.length < r + m && (g = b.substring(e - r, e) + b.substring(e, e + m), h = a.substring(0, c - r), j = a.substring(c + m), n = b.substring(0, e - r), l = b.substring(e + m)) } return g.length * 2 >= a.length ? [h, j, n, l, g] : null } if (this.Diff_Timeout <= 0) return null;
    ...
    diff_match_patch.prototype.diff_cleanupSemantic = function (a) {
        for (var b = false, c = [], d = 0, e = null, f = 0, g = 0, h = 0, j = 0, i = 0; f < a.length; ) a[f][0] == 0 ? (c[d++] = f, g = j, h = i, i = j = 0, e = a[f][1]) : (a[f][0] == 1 ? j += a[f][1].length : i += a[f][1].length, e !== null && e.length <= Math.max(g, h) && e.length <= Math.max(j, i) && (a.splice(c[d - 1], 0, [-1, e]), a[c[d - 1] + 1][0] = 1, d--, d--, f = d > 0 ? c[d - 1] : -1, i = j = h = g = 0, e = null, b = true)), f++; b && this.diff_cleanupMerge(a); this.diff_cleanupSemanticLossless(a); for (f = 1; f < a.length; ) {
            if (a[f - 1][0] == -1 && a[f][0] == 1) {
                b = a[f - 1][1];
                c = a[f][1]; d = this.diff_commonOverlap_(b, c); e = this.diff_commonOverlap_(c, b); if (d >= e) { if (d >= b.length / 2 || d >= c.length / 2) a.splice(f, 0, [0, c.substring(0, d)]), a[f - 1][1] = b.substring(0, b.length - d), a[f + 1][1] = c.substring(d), f++ } else if (e >= b.length / 2 || e >= c.length / 2) a.splice(f, 0, [0, b.substring(0, e)]), a[f - 1] = [1, c.substring(0, c.length - e)], a[f + 1] = [-1, b.substring(e)], f++; f++
            } f++
        } 
    };
    diff_match_patch.prototype.diff_cleanupSemanticLossless = function (a) {
        function b(a, b) {
            ...
            if (i || j) return 5; else if (c || d) return 4; else if (e && !g && h) return 3; else if (g || h) return 2; else if (e || f) return 1; return 0
        } for (var c = 1; c < a.length - 1; ) {
            if (a[c - 1][0] == 0 && a[c + 1][0] == 0) {
                var d = a[c - 1][1], e = a[c][1], f = a[c + 1][1], g = this.diff_commonSuffix(d, e); if (g) var h = e.substring(e.length - g), d = d.substring(0, d.length - g), e = h + e.substring(0, e.length - g), f = h + f; for (var g = d, h = e, j = f, i = b(d, e) + b(e, f); e.charAt(0) === f.charAt(0); ) {
                    d += e.charAt(0); var e = e.substring(1) + f.charAt(0), f = f.substring(1), k = b(d, e) + b(e, f); k >= i && (i = k, g =
d, h = e, j = f)
...
g), e = e.substring(0, e.length - g))), c === 0 ? a.splice(b - c - d, c + d, [1, f]) : d === 0 ? a.splice(b - c - d, c + d, [-1, e]) : a.splice(b - c - d, c + d, [-1, e], [1, f]), b = b - c - d + (c ? 1 : 0) + (d ? 1 : 0) + 1) : b !== 0 && a[b - 1][0] == 0 ? (a[b - 1][1] += a[b][1], a.splice(b, 1)) : b++, c = d = 0, f = e = ""
        } a[a.length - 1][1] === "" && a.pop(); c = false; for (b = 1; b < a.length - 1; ) a[b - 1][0] == 0 && a[b + 1][0] == 0 && (a[b][1].substring(a[b][1].length - a[b - 1][1].length) == a[b - 1][1] ? (a[b][1] = a[b - 1][1] + a[b][1].substring(0, a[b][1].length - a[b - 1][1].length), a[b + 1][1] = a[b - 1][1] + a[b + 1][1], a.splice(b - 1, 1), c =
true) : a[b][1].substring(0, a[b + 1][1].length) == a[b + 1][1] && (a[b - 1][1] += a[b + 1][1], a[b][1] = a[b][1].substring(a[b + 1][1].length) + a[b + 1][1], a.splice(b + 1, 1), c = true)), b++; c && this.diff_cleanupMerge(a)
...
f[g]);
    ...
    diff_match_patch.prototype.match_bitap_ = function (a, b, c) {
        function d(a, d) { var e = a / b.length, g = Math.abs(c - d); return !f.Match_Distance ? g ? 1 : e : e + g / f.Match_Distance } if (b.length > this.Match_MaxBits) throw Error("Pattern too long for this browser."); var e = this.match_alphabet_(b), f = this, g = this.Match_Threshold, h = a.indexOf(b, c); h != -1 && (g = Math.min(d(0, h), g), h = a.lastIndexOf(b, c + b.length), h != -1 && (g = Math.min(d(0, h), g))); for (var j = 1 << b.length - 1, h = -1, i, k, p = b.length + a.length, q, t = 0; t < b.length; t++) {
            i = 0; for (k = p; i < k; ) d(t, c +
k) <= g ? i = k : p = k, k = Math.floor((p - i) / 2 + i); p = k; i = Math.max(1, c - k + 1); var o = Math.min(c + k, a.length) + b.length; k = Array(o + 2); for (k[o + 1] = (1 << t) - 1; o >= i; o--) { var v = e[a.charAt(o - 1)]; k[o] = t === 0 ? (k[o + 1] << 1 | 1) & v : (k[o + 1] << 1 | 1) & v | (q[o + 1] | q[o]) << 1 | 1 | q[o + 1]; if (k[o] & j && (v = d(t, o - 1), v <= g)) if (g = v, h = o - 1, h > c) i = Math.max(1, 2 * c - h); else break } if (d(t + 1, c) > g) break; q = k
        } return h
    };
...
c.length + d.length; a.length2 += c.length + d.length
    ...
    diff_match_patch.prototype.patch_make = function (a, b, c) {
        ...
        if (b.length === 0) return []; for (var c = [], a = new diff_match_patch.patch_obj, e = 0, f = 0, g = 0, h = d, j = 0; j < b.length; j++) {
            var i = b[j][0], k = b[j][1]; if (!e && i !== 0) a.start1 = f, a.start2 = g; switch (i) {
...
e && (this.patch_addContext_(a, h), c.push(a), a = new diff_match_patch.patch_obj, e = 0, h = d, f = g)
    ...
    diff_match_patch.prototype.patch_apply = function (a, b) {
        if (a.length == 0) return [b, []]; var a = this.patch_deepCopy(a), c = this.patch_addPadding(a), b = c + b + c; this.patch_splitMax(a); for (var d = 0, e = [], f = 0; f < a.length; f++) {
            ...
            if (j == -1) e[f] = false, d -= a[f].length2 - a[f].length1; else if (e[f] = true, d = j - g, g = i == -1 ? b.substring(j, j + h.length) : b.substring(j, i + this.Match_MaxBits), h == g) b = b.substring(0, j) + this.diff_text2(a[f].diffs) + b.substring(j + h.length); else if (g = this.diff_main(h, g, false), h.length > this.Match_MaxBits && this.diff_levenshtein(g) / h.length > this.Patch_DeleteThreshold) e[f] = false; else {
                this.diff_cleanupSemanticLossless(g); for (var h = 0, k, i = 0; i < a[f].diffs.length; i++) {
                    var p = a[f].diffs[i]; p[0] !== 0 && (k = this.diff_xIndex(g, h)); p[0] ===
1 ? b = b.substring(0, j + k) + p[1] + b.substring(j + k) : p[0] === -1 && (b = b.substring(0, j + k) + b.substring(j + this.diff_xIndex(g, h + p[1].length))); p[0] !== -1 && (h += p[1].length)
                } 
            } 
    ...
    diff_match_patch.prototype.patch_splitMax = function (a) {
        for (var b = this.Match_MaxBits, c = 0; c < a.length; c++) if (a[c].length1 > b) {
            var d = a[c]; a.splice(c--, 1); for (var e = d.start1, f = d.start2, g = ""; d.diffs.length !== 0; ) {
                var h = new diff_match_patch.patch_obj, j = true; h.start1 = e - g.length; h.start2 = f - g.length; if (g !== "") h.length1 = h.length2 = g.length, h.diffs.push([0, g]); for (; d.diffs.length !== 0 && h.length1 < b - this.Patch_Margin; ) {
                    var g = d.diffs[0][0], i = d.diffs[0][1]; g === 1 ? (h.length2 += i.length, f += i.length, h.diffs.push(d.diffs.shift()),
j = false) : g === -1 && h.diffs.length == 1 && h.diffs[0][0] == 0 && i.length > 2 * b ? (h.length1 += i.length, e += i.length, j = false, h.diffs.push([g, i]), d.diffs.shift()) : (i = i.substring(0, b - h.length1 - this.Patch_Margin), h.length1 += i.length, e += i.length, g === 0 ? (h.length2 += i.length, f += i.length) : j = false, h.diffs.push([g, i]), i == d.diffs[0][1] ? d.diffs.shift() : d.diffs[0][1] = d.diffs[0][1].substring(i.length))
                } g = this.diff_text2(h.diffs); g = g.substring(g.length - this.Patch_Margin); i = this.diff_text1(d.diffs).substring(0, this.Patch_Margin);
                i !== "" && (h.length1 += i.length, h.length2 += i.length, h.diffs.length !== 0 && h.diffs[h.diffs.length - 1][0] === 0 ? h.diffs[h.diffs.length - 1][1] += i : h.diffs.push([0, i])); j || a.splice(++c, 0, h)
            } 
        } 
    ...
    diff_match_patch.prototype.patch_fromText = function (a) {
        var b = []; if (!a) return b; for (var a = a.split("\n"), c = 0, d = /^@@ -(\d+),?(\d*) \+(\d+),?(\d*) @@$/; c < a.length; ) {
